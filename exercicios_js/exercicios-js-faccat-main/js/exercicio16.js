function executarExercicio16() {
    alert(`Exercicio 16)\n As maçãs custam R$ 1,30 cada se forem compradas menos de uma dúzia, e R$ 1,00 se forem  compradas pelo menos 12. Escreva um programa que leia o número de maçãs compradas, calcule e  escreva o custo total da compra`)

    let macasCompradas = parseFloat(prompt(`Digite a quantidade de maçãs compradas: `))

    let custoTotal

    if (macasCompradas <= 12) {
        custoTotal = (macasCompradas * 1.30)
    } else {
        custoTotal = (macasCompradas * 1.00)
    }

    alert(`O custo total é: ${custoTotal}`)
}